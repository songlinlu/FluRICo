...
def process_group_corr_mats(
    ...
    """
    修改版：
      - 返回的矩阵 index 格式改为 "{菌名}_{组名}" (例如 "Bacteroides_HC")
      - 这样即使 shuffle_after_stack=True，也能找回每一行的身份
    """
        ...
        if isinstance(M, pd.DataFrame):
            ...
            if df.columns is None or len(df.columns) != len(taxa):
                if df.shape[1] != len(taxa):
                    raise ValueError("矩阵列数与 taxa 长度不一致，无法对齐。")
                df.columns = list(taxa)
            else:
                if set(df.columns) != set(taxa):
                    raise ValueError("分组矩阵的 columns 集合与全局矩阵不一致，无法对齐。")
                df = df.loc[:, taxa]

            if df.shape[0] != len(taxa):
                raise ValueError("矩阵行数与 taxa 长度不一致，无法设为方阵。")
            df.index = list(taxa)
            return df

        arr = np.asarray(M)
        if arr.shape[0] != arr.shape[1]:
            raise ValueError("输入 ndarray 不是方阵。")
        if arr.shape[0] != len(taxa):
            raise ValueError("输入 ndarray 尺寸与全局 taxa 长度不一致。")
    ...
    def _standardize_offdiag(df, mode: str):
        ...
        if mode == "zscore":
            ...
            raise ValueError("standardize 只能是 None/'zscore'/'robust_z'/'rank_normal'。")

        return pd.DataFrame(A, index=df.index, columns=df.columns)

    # --- taxa 基准：优先用 R_all.columns；没有就用 0..N-1 ---
...
def run_trajectory_analysis_ultimate(
    ...
    # --- 图3（microbe 才用）：数量最多（taxonomy 计数 TopN） ---
    tax_level: str = "f__",
    top_n: int = 10,
    ...
    top_n_change: int = 10,
    change_tax_level: str = None,          # microbe 用；metabolite 忽略
    comparison_groups: tuple = ("HC", "IA"),  # 默认只比较HC和IA
    ...
    """
    需求实现：
    1) 图1/图2保持不变（分析/降维/拟时序流程不改）
    2) 图3：数量最多的（仅 microbe；按 taxonomy 计数 TopN）
    3) 图4：伪时间变化最大的（只计算HC→IA，选择变化最大的top）
       - microbe：按 taxonomy 聚合后变化最大（保持原逻辑）
       - metabolite：按 Entity（代谢物名）自身变化最大（不走 taxonomy）
    4) microbe：画 1/2/3/4；metabolite：只画 1/2/4
    5) 新增绘图增强：
       - 图4：对"变化最大TopN"的每个特征，按 HC→MIA→IA 用虚线连线，并用箭头指向
         * microbe：特征=taxonomy（用该 taxonomy 在各组的 UMAP 均值点连线）
         * metabolite：特征=Entity（用该 metabolite 在各组的 UMAP 均值点连线）
       - 图1：改成 HC→MIA→IA 的箭头（质心保留），不显示原来的 PAGA 连线（只画箭头）
       - 图1：质心更大，箭头不盖过质心，从质心半径外开始
       - 图4：箭头透明度降低，不盖过点，指向点的边缘；点的透明度0.9，top点画在最上层
       - 图4：top颜色鲜明，使用离散的鲜艳颜色
    """
    ...
    def _draw_arrow(ax, p0, p1, color="k", lw=1.8, ls="-", alpha=0.9, ms=12, z=10, shrink_start=0, shrink_end=0):
        """在 ax 上画 p0->p1 的箭头（可虚线），shrink_start/end 用于避开点"""
        ...
        arrow = FancyArrowPatch(
            ...
            color=color,
            alpha=alpha,
            shrinkA=shrink_start,  # 起点缩进，避开起始点
            shrinkB=shrink_end,    # 终点缩进，避开目标点
            zorder=z,
        )
        ax.add_patch(arrow)

    def _draw_group_flow_arrows(ax, group_to_xy, order, color="k", lw=2.2, ms=16, z=12, shrink=15):
        """按 order（HC→MIA→IA）画组质心箭头，shrink 避开质心点"""
    ...
    def _draw_feature_trajectories(ax, df_centroids, order, color="k", lw=1.2, ms=10, alpha=0.5, shrink=8):
        ...
        对每个 feature：按 order 连线并画箭头（虚线），shrink 避开点
        """
            ...
            for a, b in zip(order[:-1], order[1:]):
                if a in g2p and b in g2p:
                    ...
                    ax.plot(
                        ...
                        linewidth=lw,
                        color=color,
                        alpha=alpha * 0.5,  # 连线比箭头更淡
                        zorder=8,  # 降低zorder，确保不盖过点
                    )
                    # 箭头
    ...
    print(f"矩阵类型: {'对称矩阵/相关矩阵堆叠' if is_symmetric else '非对称矩阵 (样本×特征)'}")
    print(f"数据形状: {n_obs} 行 × {n_vars} 列")
    print(f"{'='*60}\n")

    # ----------------------------
    # A) 分组标签：从 index 解析（去除顺序依赖）
    # ----------------------------
    ...
    if group_labels_input is not None:
        if len(group_labels_input) != n_obs:
            raise ValueError(f"group_labels_input 长度 ({len(group_labels_input)}) 必须等于数据行数 ({n_obs})")
        group_labels = list(map(str, group_labels_input))
        print("✓ 使用手动指定的分组标签 group_labels_input")
    else:
        ...
        if len(unknown) > 0:
            raise ValueError(
                f"从 df.index 解析分组失败：{len(unknown)}/{n_obs} 行为 Unknown。\n"
                f"示例: {unknown[:10]}\n"
                f"请保证 index 直接等于 {violin_order}，或形如 xxx_HC/xxx-MIA/xxx|IA，或手动传 group_labels_input。"
            )
        print("✓ 分组标签从 df.index 解析（已去除行顺序依赖）")

    print(f"  分组统计: {dict(pd.Series(group_labels).value_counts())}")
    ...
    if mask_self_corr:
        ...
        if hit_rate < self_corr_min_match_rate:
            print(
                "⚠ Self-corr 匹配率偏低：说明 df.index 的实体身份与 df.columns 可能不对齐。\n"
                "  若你的 index 仅包含 HC/MIA/IA（不含实体名），建议关闭 mask_self_corr。"
            )
    ...
    # 统一实体名（microbe=菌名；metabolite=代谢物名）
    adata.obs["Entity"] = [_canon_entity_name(rn) for rn in raw_index_names]

    # ----------------------------
    # D) microbe taxonomy（用于图3/图4 microbe 分支）
    # ----------------------------
    ...
    if data_type == "microbe":
        print(f"正在解析 Taxonomy (层级: {tax_level})...")
        row_taxs_raw = []
        ...
        print("data_type=metabolite：跳过 Taxonomy（图3不画，图4不依赖 taxonomy）。")
    ...
    if root_strategy == "centroid":
        ...
        print(f"✓ Root策略: {root_group} 组中心点样本 (索引: {closest_idx})")
        print(f"  → 样本名: {raw_index_names[closest_idx]} | Entity: {adata.obs['Entity'].iloc[closest_idx]}")
    else:
        target_ent = str(root_strategy).strip()
        print(f"正在寻找 Root 样本: Entity='{target_ent}', 组别='{root_group}'...")
        found_idx = -1
        ...
        if found_idx == -1:
            raise ValueError(
                f"Root策略失败: 在 {root_group} 组中未找到名为 '{target_ent}' 的实体。\n"
                f"请检查输入名称是否与 df.index (去除后缀后) 匹配。"
            )
        adata.uns["iroot"] = int(found_idx)
        print(f"✓ Root策略: 指定实体 '{target_ent}' ({root_group}) 作为起点 (索引: {found_idx})")

    sc.tl.dpt(adata)
    sc.tl.umap(adata, random_state=0)

    # ----------------------------
    # F) PAGA（计算保留；绘图不再画原连线）
    # ----------------------------
    print("计算 PAGA 拓扑...")
    ...
    # ----------------------------
    # G) 图4：伪时间变化最大的（只计算HC→IA，使用abs_delta选择最大变化）
    # ----------------------------
    change_taxa_info = None   # microbe 用
    change_entity_info = None # metabolite 用
    ...
    if plot_change:
        ...
        print(f"✓ 图4: 计算伪时间变化 ({start_group} → {end_group})，使用 {change_metric} 指标")
        ...
        if data_type == "microbe":
            ...
            df_pt = pd.DataFrame({
                ...
                "entity": adata.obs["Entity"].values,  # 添加原始特征名
            })
            df_pt = df_pt[df_pt["taxonomy"] != "Others"]

            if df_pt.shape[0] == 0:
                print("⚠ 图4跳过：Taxonomy_Change 全为 Others（index 不含菌名或 full_name_dic 无对应）。")
                plot_change = False
            else:
                ...
                # 添加 original_features 列（每个 taxonomy 对应的所有原始菌名）
                ...
                if start_group not in pivot.columns or end_group not in pivot.columns:
                    print(f"⚠ 图4跳过：比较组 {comparison_groups} 在数据中不完整。")
                    plot_change = False
                else:
                    # 只计算HC→IA的变化
                    if change_metric == "delta":
                        ...
                        raise ValueError(f"未知的 change_metric: {change_metric}")

                    pivot = pivot.dropna(subset=["change"])
                    if pivot.shape[0] == 0:
                        print("⚠ 图4跳过：无可用 taxonomy 变化条目。")
                        plot_change = False
                    else:
                        # 选择变化最大的top（按绝对变化量排序）
                        ...
                        # 为图4准备"taxonomy 轨迹的 UMAP 质心点"（按HC→MIA→IA顺序）
                        rows = []
                        ...
                        for tax in sorted_taxa:
                            for g in violin_order:  # 确保按HC→MIA→IA顺序
            ...
            # metabolite：Entity 聚合，完全与 taxonomy 无关
            df_pt = pd.DataFrame({
            ...
            if start_group not in pivot.columns or end_group not in pivot.columns:
                ...
                if pivot.shape[0] == 0:
                    ...
                    # 为图4准备"entity 轨迹的 UMAP 质心点"（按HC→MIA→IA顺序）
                    rows = []
                    umap_xy = adata.obsm["X_umap"]
                    obs_group = adata.obs[group_col].astype(str).values
    ...
    if plot:
        ...
        # microbe：1/2/3/4；metabolite：1/2/4（3不画）
        if data_type == "microbe":
            n_plots = 4 if (plot_change and change_taxa_info is not None) else 3
        else:
            n_plots = 3 if (plot_change and change_entity_info is not None) else 2

        fig, axs = plt.subplots(1, n_plots, figsize=(6 * n_plots, 4), dpi=500)
        if n_plots == 1:
            axs = [axs]
        ...
        # 图1：Trajectory（质心更大，箭头避开质心）
        sc.pl.umap(
            adata, color=group_col, ax=axs[0], show=False,
        ...
        # 图2：Pseudotime（保持不变）
        sc.pl.umap(
            adata, color="dpt_pseudotime", color_map=cmap, ax=axs[1], show=False,
        ...
        if data_type == "microbe":
            ...
            sc.pl.umap(
                adata, color="Taxonomy_Plot", ax=axs[2], show=False, alpha=0.5,
                title=title3, frameon=False
            )
            leg3 = axs[2].get_legend()
            if leg3 is not None:
            ...
            # 图4：伪时间变化最大的（新逻辑：alpha=0.9, top点在最上层，鲜艳颜色，箭头按HC→MIA→IA）
            if plot_change and change_taxa_info is not None:
            ...
            # metabolite：只画 1/2/4（4与taxonomy无关，新逻辑同上）
            if plot_change and change_entity_info is not None:
                if change_metric == "fold":
                    change_word = "Fold Change"
                elif change_metric == "abs_delta":
                    change_word = "Absolute Change"
    ...
    print("✓ 分析完成！")
    print(f"{'='*60}\n")
    # 检查 pivot 是否存在，不存在则返回 None 或空 Dataframe
...
def plot_pseudotime_violin_and_ecdf(
    ...
    dpi=500,
    figsize=(12, 4),
    # ---- 对齐“参考函数”风格：默认不透明 + 全框 + 有 legend 框 + grid ----
    transparent=False,
    ecdf_linewidth=2.0,
    median_vline=True,
    ...
    """
    左：Violin（Scanpy），右：ECDF（Matplotlib）
    风格对齐参考函数：
      - 默认不透明背景（transparent=False）
      - 不隐藏 spines（保留全框）
      - legend 带框，位置 lower right，fontsize=9
      - grid(alpha=0.2)
      - tight_layout + savefig
    """

    # --- 显式颜色映射（只由 group_order 决定） ---
    color_map = {g: c for g, c in zip(group_order, group_colors)}

    # --- 1) 从 obs 中显式取列 ---
    if group_col not in adata.obs.columns:
        raise KeyError(f"adata.obs 中不存在列 '{group_col}'")
    if pseudotime_key not in adata.obs.columns:
        raise KeyError(f"adata.obs 中不存在列 '{pseudotime_key}'")
    ...
    # --- 2) present_groups：严格按 group_order 顺序筛选存在的组 ---
    present_groups = [g for g in group_order if g in set(dfp[group_col].values)]
    if len(present_groups) == 0:
        raise ValueError("dfp 中没有任何 group_order 指定的组别，无法绘图。")

    # --- 3) 给 scanpy violin 用：构造临时 view，只保留 present_groups ---
    ...
    # 参考风格：legend 有框、lower right、fontsize=9（不要 frameon=False）
...
def map_test_to_train_within_group_knn(
    ...
    """
    组内 KNN 映射 + 自动绘图（与训练集格式完全一致）
    
    功能：
    1. 组内KNN映射：每个 test 点只在 train 的同组里找邻居
    2. 映射伪时间：用邻居的 train dpt_pseudotime 加权平均
    3. 映射UMAP：同样加权平均得到 test 的 X_umap（保持 train 坐标系）
    4. 自动绘图：与训练集完全一致的图（4子图UMAP + Violin/ECDF）
    ...
    if "dpt_pseudotime" not in adata_train.obs:
        raise ValueError("adata_train 缺少 obs['dpt_pseudotime']")
    if map_umap and ("X_umap" not in adata_train.obsm):
        raise ValueError("adata_train 缺少 obsm['X_umap']")
    ...
    if rep == "X_pca":
        if "X_pca" not in adata_train.obsm:
            raise ValueError("rep='X_pca' 但 adata_train.obsm['X_pca'] 不存在")
        if "X_pca" not in adata_test.obsm:
            raise ValueError("rep='X_pca' 但 adata_test.obsm['X_pca'] 不存在")
        ...
        raise ValueError("rep 只能是 'X_pca' 或 'X'")
    ...
    if missing_in_train:
        raise ValueError(f"test 出现 train 不包含的组：{missing_in_train}")
    ...
    for g in train_groups:
        ...
        if tr_idx.size == 0:
            raise ValueError(f"train 组 {g} 为空")
    ...
    if plot:
        ...
        if plot_type in ["umap", "both"]:
            ...
            if savepath_umap:
                ...
                print(f"✓ UMAP图已保存: {savepath_umap}")
            plt.show()
...
def _plot_pseudotime_violin_and_ecdf(
    ...
    """
    Violin + ECDF（风格对齐参考函数）：
      - 取消隐藏 spines（保留全框）
      - legend 默认带框、lower right、fontsize=9
      - grid(alpha=0.2)
      - tight_layout + savefig
      - 其它逻辑不变（顺序/分类/颜色稳定）
    """
    ...
    if len(present_groups) == 0:
        raise ValueError("dfp 中没有任何 violin_order 指定的组别")
    ...
    axs[1].set_xlabel("Pseudotime")
    axs[1].set_ylabel("Cumulative proportion")
    axs[1].legend(loc="lower right", fontsize=9,frameon=True)  # ✅ 带框（默认 frameon=True）
    axs[1].grid(alpha=0.2)

    # ✅ 不再隐藏 top/right spine：保留全包框（与参考函数一致）
    plt.tight_layout()
    if savepath:
        plt.savefig(savepath, dpi=dpi, transparent=transparent)
        print(f"✓ Violin图已保存: {savepath}")
    plt.show()
